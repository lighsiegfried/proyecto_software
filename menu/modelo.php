...
class menu_modelo{
    ...
    function get_user(){ //lista de usuarios de PRODUCCION
        global $pdo;
        $qry="
    ...
    function get_kardex_procesos(){

        $qry="
    select 
        id_copia,t6.codigo_barra,
        (
                    case
    ...
    if (timestampdiff(hour, fecha_solicitud, fecha_usuario) <= 23, 
            ...
            (
            case
                    when timestampdiff(day, fecha_solicitud, fecha_usuario) = 1 then concat('1 día y ' , timestampdiff(hour, fecha_solicitud, fecha_usuario) - (timestampdiff(day, fecha_solicitud, fecha_usuario) * 24),' horas')
                    when timestampdiff(month, fecha_solicitud, fecha_usuario) = 0 then concat(timestampdiff(day, fecha_solicitud, fecha_usuario), ' días y ', timestampdiff(hour, fecha_solicitud, fecha_usuario) - (timestampdiff(day, fecha_solicitud, fecha_usuario) * 24),' horas')
                    when timestampdiff(month, fecha_solicitud, fecha_usuario)  = 1 then '1 mes'
else concat(timestampdiff(month, fecha_solicitud, fecha_usuario),' meses y ',timestampdiff(day, fecha_solicitud, fecha_usuario) - (timestampdiff(month, fecha_solicitud, fecha_usuario) * 30),' días')
            end
            )) as tiempo_transcurrido,DATE(fecha_solicitud)
from 
(  
    /*Encabezado*/
    select id_copia,id_codigo_barra, pedido,bodega,proceso_bodega,estado,actual,tipo_orden,id_ruta,accion_observacion,fecha_solicitud,activo, cod_usuario,fecha_usuario from kardex_procesos 
) t1 left join
(	 
    /*Nombres de usuarios*/
...
(
    /*codigo de barra*/
    select id_codigo_barra as id_codigo_barramain , codigo_barra from mp_codigo_barra 
) t6 on t1.id_codigo_barra = t6.id_codigo_barramain
